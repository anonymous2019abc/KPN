...
def visualize_gt(image, contours, tr=None):
    ...
    if tr is not None:
        ...
        tr_map = cav.heatmap(np.array(tr / 1 * 255, dtype=np.uint8))
        h, w = tr_map.shape[:2]
        tr_map = cv2.resize(tr_map, (w*cfg.scale, h*cfg.scale))
        image_show = np.concatenate([image_show, np.array(tr_map*255, dtype=np.uint8), np.array(tr_map*255, dtype=np.uint8)], axis=1)
        return image_show
    else:
        return image_show


def visualize_detection(image, contours, tr=None, pred_mask=None):
    image_show = image.copy()
    image_show = np.ascontiguousarray(image_show[:, :, ::-1])
    hi, wi, ci = image_show.shape
    ...
    if tr is not None:
        ...
        h,w = tr_map.shape[:2]
        tr_map = cv2.resize(tr_map, (w*cfg.scale, h*cfg.scale))
        tr_map = tr_map[0:hi, 0:wi, :]
        if pred_mask is not  None:
            pred_mask_map = cav.heatmap(np.array(pred_mask / 1 * 255, dtype=np.uint8))
            h,w = pred_mask_map.shape[:2]
            pred_mask_map = cv2.resize(pred_mask_map, (w*cfg.scale, h*cfg.scale))
            pred_mask_map = pred_mask_map[0:hi, 0:wi, :]
        else:
            pred_mask_map = tr_map
        #print(tr_map.shape)
        #print(image_show.shape)
        image_show = np.concatenate([image_show, np.array(pred_mask_map*255, dtype=np.uint8), np.array(tr_map*255, dtype=np.uint8)], axis=1)
        return image_show
    else:
        return image_show
